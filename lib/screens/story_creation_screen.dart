...
class _StoryCreationScreenState extends State<StoryCreationScreen>
  ...
  Future<void> _updateDescription() async {
    ...
    try {
      await _provider
          .updateStory(_createdStory!.copyWith(description: desc));
      _createdStory = _createdStory!.copyWith(description: desc);
    } catch (e) {
      // Silent — description save is best effort
    }
  }
  ...
  Future<void> _updateGenre(String? genre) async {
    ...
    try {
      await _provider.updateStory(
          _createdStory!.copyWith(genre: genre?.toLowerCase()));
      _createdStory =
          _createdStory!.copyWith(genre: genre?.toLowerCase());
    } catch (e) {
      // Silent
    ...
    if (_pages.length > 1) {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text('Failed to delete page'),
              backgroundColor: const Color(0xFFD4183D),
              behavior: SnackBarBehavior.floating,
